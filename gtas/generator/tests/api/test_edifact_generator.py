...
class EdifactGeneratorTest(TestCase):
    ...
    def test_single_segment_output(self):
        messages = []
        # message = Message()
        # edifact_message.append(message.add_segment(Segment('BGM', ['745'])).serialize())
        # edifact_message.append(message.add_segment(Segment('NAD', ['MS', '', ['', 'WORLD CUSTOMS ORGANIZATION BRU']])).serialize())
        ...
        # self.assertEqual(message.serialize(), "BGM+745'")
