...
from gtas.parsers.paxlst import edifact as edifact
...
class PrettyPrintHandler(edifact.Handler):
    def visit_codeset_element(
        ...
        print(
            "{} {}: '{}' ({})".format(
                data_element_schema["code"], desc, codeset_value, element
            )
        )
        return False, None
...
class JsonPrintHandler(edifact.Handler):
    ...
    def visit_unknown_segment(self, segment):
        self._current_segment = {"code": segment.tag, "attributes": []}
    ...
    def _visit_unknown_element(self, element):
        result = {"code": element["code"], "value": element["value"]}
        self._current_segment["attributes"].append(result)
